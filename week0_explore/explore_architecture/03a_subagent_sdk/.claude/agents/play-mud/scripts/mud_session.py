...
class MudLink:
    ...
    def open(self) -> None:
        ...
        # `create_connection(..., timeout=10)` leaves the socket's timeout permanently
        # set to 10s (it's not just a connect-attempt timeout). Left in place, the
        # reader thread's blocking recv() below raises socket.timeout (a subclass of
        # OSError) after any 10s stretch of MUD-side silence -- indistinguishable from
        # a real close to the `except OSError: pass` below, which then marks the link
        # closed even though the TCP connection is still perfectly alive. Reset to
        # blocking mode (no timeout) now that the connection is established.
    ...
    def login(self, username: str, password: str, sex: str | None = None, char_class: str | None = None) -> None:
        """Drive login for an existing account, or creation of a brand-new one.

        The server enforces a short idle timeout on the unauthenticated
        name/password/sex/class prompts during character creation -- an LLM
        caller pausing to reason between separate `send` calls at that stage
        was observed to get disconnected mid-creation. So when `sex` and
        `char_class` are supplied, this method drives the *entire* creation
        dialogue synchronously in one call (name confirm, password x2, sex,
        class, press-return, enter game) instead of handing control back
        between each prompt. If either is omitted, it falls back to
        confirming the name spelling and returning immediately, leaving the
        rest for the caller to drive manually via send/read -- slower, and
        was observed to risk a server-side disconnect, so prefer passing both."""
        self.read_until(re.compile(r"By what name.*\?", re.I | re.S))
        self.send_line(username)
        out = self.read_until(re.compile(r"Password|Did I get that right", re.I))
        if re.search("Did I get that right", out, re.I):
            self.send_line("Y")
            self.read_until(re.compile(r"Give me a password", re.I))
            self.send_line(password)
            self.read_until(re.compile(r"retype password", re.I))
            self.send_line(password)
            if sex is None or char_class is None:
                return
            self.read_until(re.compile(r"What is your sex", re.I))
            self.send_line(sex)
            self.read_until(re.compile(r"Select a class", re.I | re.S))
            self.send_line(char_class)
            self.read_until(re.compile(r"PRESS RETURN", re.I))
            self.send_line("")
            self.read_until(re.compile(r"Make your choice", re.I | re.S))
...
def daemon_main(args: argparse.Namespace) -> None:
    ...
    link.open()
    try:
        link.login(args.user, args.password, sex=args.sex, char_class=args.char_class)
...
SEX_ALIASES = {"m": "M", "male": "M", "f": "F", "female": "F"}
CLASS_ALIASES = {
    "c": "C", "cleric": "C",
    "t": "T", "thief": "T",
    "w": "W", "warrior": "W",
    "m": "M", "magic-user": "M", "magic_user": "M", "mage": "M", "magic-user": "M",
}


def normalize_choice(value: str, aliases: dict[str, str], field_name: str) -> str:
    key = value.strip().lower()
    if key not in aliases:
        raise argparse.ArgumentTypeError(
            f"unrecognized {field_name} {value!r}; expected one of {sorted(set(aliases))}"
        )
    return aliases[key]
...
def cmd_connect(args: argparse.Namespace) -> None:
    ...
    cmd = [sys.executable, os.path.abspath(__file__), "_daemon",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    connect.add_argument("--sex", default=None,
                          type=lambda v: normalize_choice(v, SEX_ALIASES, "sex"),
                          help="M/F or male/female -- only used if --user doesn't exist yet, to "
                               "create it. Pass together with --class for atomic, one-shot "
                               "character creation (recommended: the server times out the "
                               "unauthenticated creation prompts if left mid-dialogue too long).")
    connect.add_argument("--class", dest="char_class", default=None,
                          type=lambda v: normalize_choice(v, CLASS_ALIASES, "class"),
                          help="cleric/thief/warrior/magic-user (or C/T/W/M) -- see --sex.")
    add_name(connect)
    ...
    daemon.add_argument("--sex", default=None)
    daemon.add_argument("--class", dest="char_class", default=None)
